...
- 데이터 필드를 시트에 추가하는 방법 (필드 2개 기준)
    - 원하는 필드를 각각 더블 클릭 -> Tableau가 차원과 측정값에 따라 자동 시각화
    - 필드 하나를 `열` 선반으로 드래그 & 다른 하나를 `행` 선반으로 드래그 -> Tableau가 자동 시각화
    - 필드 하나를 클릭 -> `Ctrl`을 누른 채로 나머지 클릭 -> 드래그
    - 필드 하나를 `열` 선반으로 드래그 & 다른 하나를 `마크`의 `텍스트` 상자로 드래그
